...
// Note Comments for Jan, built a 
//test in the FrontEnd to check wether the Number submitted before it is turned into a hash value, that is is an integer.
...
contract GameBC3 {
    ...
    function joinGame() public payable {
        ...
        require(
            ...
            "You have already paid the entry fee, you already joined the game!"
        );
        require(
    ...
    function commitHash(bytes32 _hash) public {
        ...
        ); //Checks whether the player has already submitted a number.
        ...
        // if (playersWithHash == minNumberOfPlayers) { // third player has to pay more gas fees than all the others... everyone should play the same entryFee.
        ...
        //     startRevealPhase(); // gas fees for the last players maybe really high?! Maybe implement an external function onylOwner.
        // }
    }

    /// @dev Helper function - Starts the reveal phase of the game.
    function startRevealPhase() public onlyOwner {
        require(playersWithHash >= minNumberOfPlayers);
        revealPhase = true;
    ...
    function determineWinner() public onlyOwner {
        ...
        uint256 twoThirdAverage = (66 * totalSum) /
            (100 * playersWhoHaveRevealed.length); // In Soldity, a standard division results in a whole number rounded down to 0 (https://docs.openzeppelin.com/contracts/4.x/api/utils#SignedMath)
